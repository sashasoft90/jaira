...
- The refs are now brought up to date by themselves every ten minutes, in a detached process beside whatever command you ran, so a ticket assigned to you reaches you even if you never open the board and only ever read — no command waits for it, `fetch-every-minutes` in `~/.jaira/settings.json` changes the interval, and `JAIRA_NO_FETCH=1` turns it off.
